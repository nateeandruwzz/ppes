...
export const getEvidenceByEvaluatee = async (req, res) => {
    try {
        const { evaluatee_id } = req.params;
        const rows = await Evidence.getByEvaluatee(evaluatee_id);
        if (rows.length > 0) {
            return res.json({ status: 1, message: "สำเร็จ", data: rows });
        } else {
            return res.json({ status: 0, message: "ไม่พบข้อมูล", data: [] });
        }
    } catch (err) {
        return res.status(500).json({ status: 0, message: "เซิร์ฟเวอร์ผิดพลาด" });
    }
