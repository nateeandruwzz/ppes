...
const filteredPeriods = computed(() => {
    if (!searchQuery.value.trim()) return periods.value
    const q = searchQuery.value.trim().toLowerCase()
    return periods.value.filter(p => p.name?.toLowerCase().includes(q))
})

const totalItems = computed(() => filteredPeriods.value.length)

const paginatedPeriods = computed(() => {
    const start = (currentPage.value - 1) * itemsPerPage
    return filteredPeriods.value.slice(start, start + itemsPerPage)
})
...
<template>
    <div class="space-y-6">
        ...
        <div class="relative">
            ...
            <input v-model="searchQuery" type="text" placeholder="ค้นหารอบการประเมิน..."
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-zinc-100">
                        ...
                        <!-- Data Rows -->
                        <tr v-else v-for="(item, index) in paginatedPeriods" :key="item.id"
